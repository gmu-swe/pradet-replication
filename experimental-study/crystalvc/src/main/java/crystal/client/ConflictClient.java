...
/**
 * Conflict Client UI; displays the view showing the state of the repositories
 * contained in the preferences.
 * 
...
public class ConflictClient implements ConflictDaemon.ComputationListener {
	...
	/**
	 * A map that tells us which ImageIcon contained in a JLabel) corresponds to
	 * each DataSource on the GUI.
	 */
	...
	public void createAndShowGUI(final ClientPreferences prefs) {
		...
		// could set the manuBar layout to make the last, invisible blank menu
		// item take up all the space.
		...
		clearCache.addActionListener(new ActionListener() {
			// @Override
			public void actionPerformed(ActionEvent e) {
				int option = JOptionPane.showConfirmDialog(null, "Do you want to empty cache?", "Empty cache",
				...
				if (option == JOptionPane.YES_OPTION) {
					...
					_log.info("User selected Clear Cache from the menu. All cache has been emptied at "
							+ _preferences.getTempDirectory());
					...
					deleteRepo.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							int option = JOptionPane.showConfirmDialog(null,
									"Do you want to delete the " + source.getShortName() + "'s repository?",
	...
	private JMenuItem getClearCacheItem(final ProjectPreferences projPref) {
		JMenuItem clearProjectCacheMenu = new JMenuItem(
				"Clear " + projPref.getEnvironment().getShortName() + " project cache");
		clearProjectCacheMenu.addActionListener(new ActionListener() {
			// @Override
			public void actionPerformed(ActionEvent action) {
				int option = JOptionPane.showConfirmDialog(null,
						"Do you want to empty the " + projPref.getEnvironment().getShortName() + " cache?", "Warning",
						JOptionPane.YES_NO_OPTION);
				// TODO wait for the current refresh to finish or kill it
				if (option == JOptionPane.YES_OPTION) {
					boolean hadToDisable = _disableDaemon.getText().equals("Stop Crystal updates");
					...
					target.add(_preferences.getTempDirectory() + projPref.getEnvironment().getShortName() + "_"
							+ projPref.getEnvironment().getShortName());
	...
	private void refresh() {
		...
		for (ProjectPreferences projPref : _preferences.getProjectPreference()) {
			...
			if (actionResult.getLocalState().equals(LocalStateResult.PENDING)
					&& actionResult.getLastLocalState() != null) {
				...
				if ((actionResult == null) || (actionResult.getAction() == null))
					...
					if (actionResult.getAction().length() < "git commit".length()) {
						action.setText(actionResult.getAction()
								+ new String(new char["git commit".length() - actionResult.getAction().length()])
										.replace('\0', ' '));
					} else
						action.setText(actionResult.getAction());
				}
			...
			for (DataSource source : projPref.getDataSources()) {
				if (!(source.isHidden())) {
					 ...
					 * result.getLastRelationship();
					 * 
					 * 
					 * // if it's pending, show whatever value it had last time
					 * String tip = null; if
					 * ((relationship.getName().equals(Relationship.PENDING) ||
					 * (!(relationship.isReady()))) && lastRelationship != null)
					 * { current.setIcon(lastRelationship.getIcon()); tip =
					 * lastRelationship.getToolTipText(); } else { // otherwise,
					 * show fresh value current.setIcon(relationship.getIcon());
