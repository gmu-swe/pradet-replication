...
/**
 * The GUI that allows editing of the Crystal configuration. Basically,
 * PreferencesGUIEditorFrame lets you edit a ClientPreferences object by adding,
...
public class PreferencesGUIEditorFrame extends JFrame {
	...
	/**
	 * @return the singleton instance of this frame. If a frame already exists,
	 *         just returns that one. otherwise, it creates a new one with the
	 *         specified prefs configuration.
	 * @param prefs:
	 *            the configuration to use if no frame exists yet.
	 */
	...
	/**
	 * A private constructor to create a new editor frame based on the specified
	 * prefs configuration.
	 * 
	 * @param prefs:
	...
	private PreferencesGUIEditorFrame(final ClientPreferences prefs) {
		...
		if (copyPrefs.getProjectPreference().isEmpty()) {
			// ClientPreferences client = new ClientPreferences("/usr/bin/hg/",
			// "/tmp/crystalClient/");
			ProjectPreferences newGuy = new ProjectPreferences(
		...
		// getContentPane().add(new JLabel("Closing this window will save the
		...
		// new MyPathChooser("Path to hg executable", hgPath,
		 ...
		 * 
		 * //@Override public void stateChanged(ChangeEvent e) { frame.pack();
		...
		for (final ProjectPreferences copyPref : copyPrefs.getProjectPreference()) {
			...
			try {
				current = new ProjectPanel(copyPref, copyPrefs, frame, projectsTabs, changedComponents,
						prefs.getProjectPreferences(copyPref.getEnvironment().getShortName()), validText);
				...
				// projectsTabs.setTitleAt(projectsTabs.getTabCount() - 1,
				// current.getName());
		...
		// final JButton deleteProjectButton = new JButton("Delete This
		// Project");
		...
		newProjectButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				final ProjectPreferences newGuy = new ProjectPreferences(
						new DataSource("New Project " + --count, "", DataSource.RepoKind.HG, false, null), copyPrefs);
				try {
					copyPrefs.addProjectPreferences(newGuy);
				} catch (DuplicateProjectNameException e1) {
					// This should never happen because we just found a clean
					// project name to use.
				...
				final ProjectPanel newGuyPanel = new ProjectPanel(newGuy, copyPrefs, frame, projectsTabs,
						changedComponents, null, validText);
		...
		/*
		 * deleteProjectButton.addActionListener(new ActionListener() { public
		 * void actionPerformed(ActionEvent e) { int current =
		 * projectsTabs.getSelectedIndex(); int option =
		 * JOptionPane.showConfirmDialog(null,
		 * "Do you want to delete project \"" + projectsTabs.getTitleAt(current)
		 * + "\"?", "Empty cache", JOptionPane.YES_NO_OPTION); // TODO wait for
		 * the current refresh to finish or kill it if(option ==
		 * JOptionPane.YES_OPTION) {
		 * prefs.removeProjectPreferencesAtIndex(current);
		 * projectsTabs.remove(current); if
		 * (prefs.getProjectPreference().isEmpty())
		 * deleteProjectButton.setEnabled(false); prefs.setChanged(true);
		 * frame.pack(); } } });
		 */
			...
			public void actionPerformed(ActionEvent e) {
				if (copyPrefs.hasChanged() || changedComponents.values().contains(true)) {
					...
					if (!validText.values().contains(false)) {
						...
						JOptionPane.showMessageDialog(null, "You have invalid input.", "Warning",
		...
		addWindowListener(new WindowListener() {
			public void windowClosing(WindowEvent arg0) {
				setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
				if (copyPrefs.hasChanged() || changedComponents.values().contains(true)) {
					int n = JOptionPane.showConfirmDialog(null, "Do you want to save your data?", "Saving data",
							JOptionPane.YES_NO_CANCEL_OPTION);
					if (n == JOptionPane.YES_OPTION) {
						/*
						 * for (JTextField field : validText.keySet()) {
						 * if(validText.get(field) == false) {
						 * System.out.println(field.getText() + ": bad"); } else
	...
	private class DeleteProjectButton extends JButton {
		...
		public DeleteProjectButton(final ClientPreferences prefs, final JTabbedPane projectsTabs, final JFrame frame,
				final ProjectPanel newGuyPanel, final ProjectPreferences newGuy) {
			super("X");
			...
			addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					int option = JOptionPane.showConfirmDialog(null,
							"Do you want to delete project \"" + targetProject + "\"?", "Delete Project",
