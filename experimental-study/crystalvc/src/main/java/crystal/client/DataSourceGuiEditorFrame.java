...
/**
 * Class DataSourceGuiEditorFrame allows adding or editing new repository to a
 * project
 * 
 * @author Haochen
 *
...
public class DataSourceGuiEditorFrame extends JFrame {
	 ...
	 * 
	 * @param prefs
	 *            client preferences containing the project preferences.
	 * @param pref
	 *            project preference containing the data source
	 */
	 ...
	 *            project preference containing the data source
	 * @param sourceName
	 *            data source name to edit: create new data source if it is null
	 */
	public DataSourceGuiEditorFrame(final ClientPreferences prefs, final ProjectPreferences pref, String sourceName) {
		...
		/*
		 * if(prefs.getProjectPreference().contains(pref)){ throw new
		 * IllegalArgumentException(
		 * "Input project preference is not contained in the " +
		 * "client preferences."); }
		 */
		...
		if (isAddRepo) {
			...
			throw new IllegalArgumentException("Illegal source name not contained in " + "projectPreference.");
		}

		setIconImage((new ImageIcon(Constants.class.getResource("/crystal/client/images/crystal-ball_blue_128.jpg")))
				.getImage());
		...
		shortName.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				try {
					 ...
					 * "Invalid input for project name.", "Warning",
		...
		cloneString.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				try {
					...
					/*
					 * JOptionPane.showMessageDialog(null,
					 * "Invalid input for clone address.", "Warning",
					 * JOptionPane.ERROR_MESSAGE);
		...
		parent.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				try {
					...
					/*
					 * JOptionPane.showMessageDialog(null,
					 * "Invalid input for parent.", "Warning",
					 * JOptionPane.ERROR_MESSAGE);
		...
		SpringLayoutUtility.formGridInColumn(sourcePanel, sourcePanel.getComponents().length / SOURCE_COLUMNS,
				SOURCE_COLUMNS);
		...
		if (isAddRepo) { // add new repository: use add button
			...
			saveButton.addActionListener(new ActionListener() {
				...
				// @Override
				public void actionPerformed(ActionEvent arg0) {
					if (pref.getDataSources().contains(_source.getShortName())) { // new
																					// name
																					// is
																					// a
																					// duplicate
						JOptionPane.showMessageDialog(null, "Short name already exists in the project repository.",
								"Warning", JOptionPane.ERROR_MESSAGE);
					} else {
						try {
							ClientPreferences.savePreferencesToDefaultXML(prefs);
			...
			public void windowClosing(WindowEvent arg0) {
				...
				if (prefs.hasChanged()) {
					...
					if (n == JOptionPane.YES_OPTION) {
						...
						if (pref.getDataSources().contains(_source)) { // new
																		// name
																		// is a
																		// duplicate
							JOptionPane.showMessageDialog(null,
