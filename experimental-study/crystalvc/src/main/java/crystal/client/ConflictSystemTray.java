...
/**
 * The system tray icon UI. (This lives in the title bar in OS X or somewhere
 * else in Linux). This UI contains a few menu options and allows opening up the
 * larger window UI. If the system tray is not supported, the UI switches to a
 * window-only view.
 * 
...
public class ConflictSystemTray implements ComputationListener {
	...
	// A menu element that dictates whether Crystal ConflictDeamon is running
	// (refreshing).
	...
	// The other menu elements are not referenced from listeners, so they are
	// declared only locally.

	/**
	 * Constructs a brand new Crystal system tray icon, if the OS allows it. If
	 * the OS does not allow it, creates an empty tray icon object holding some
	 * nulls.
	 */
	private ConflictSystemTray() {
		...
		if (TRAY_SUPPORTED) {
			...
			// _trayIcon = new TrayIcon((new
			// ImageIcon(Constants.class.getResource("/crystal/client/images/bulb.gif"))).getImage());
			_trayIcon = new TrayIcon(
					(new ImageIcon(Constants.class.getResource("/crystal/client/images/crystal-ball_blue_32.png")))
							.getImage());
		} else {
			_tray = null;
			_trayIcon = null;
	...
	/**
	 * A listener on the about menu item. When the user clicks on "about", a
	 * dialog pops up with some info on Crystal.
	 */
	public void aboutAction() {
		JOptionPane.showMessageDialog(null,
				...
				"Crystal: Proactive Conflict Detector for Distributed Version Control", JOptionPane.PLAIN_MESSAGE,
	...
	public void loadPreferences() {
		try {
			...
			String dialogMessage = "The preferences file ( " + ClientPreferences.CONFIG_PATH
			...
			int answer = JOptionPane.showConfirmDialog(null, dialogMessage, "Invalid configuration file",
			...
			if (answer == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(editorFrame,
						"Please remember to restart the client after closing the configuraton editor.");
				// and disable client
	...
	private void createAndShowGUI() {
		// Create components for a popup menu components to be used if System
		// Tray is supported.
		...
		try {
			if (!(RunIt.validHG(Constants.MIN_HG_VERSION, _prefs.getHgPath(), _prefs.getTempDirectory()))) {
				JOptionPane.showMessageDialog(null,
						"Your computer is running an outdated version of hg.\nThe must be running at least version "
								+ Constants.MIN_HG_VERSION,
		...
		if (TRAY_SUPPORTED) {
			...
			_trayIcon.setImage(
					(new ImageIcon(Constants.class.getResource("/crystal/client/images/16X16/must/clock.png")))
							.getImage());
			...
			_trayIcon.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent ae) {
					...
					// doesn't work on OS X; it doesn't register double clicks
					// on the tray
					showClient();
				}
			});
	...
	/**
	 * Creates and starts a new timer (throws away the old one). The timer fires
	 * an update every refresh seconds, unless there is a pending task.
	 */
	private void createTimer() {

		// note that the timer works in milliseconds and the argument is in
		// seconds
	...
	/**
	 * If the deamon is not running, does nothing. If the deamon is running,
	 * creates a new executor and performs the calculations on all repos of all
	...
	public void performCalculations() {
		...
		// final CalculateLocalStateTask clst = new
		// CalculateLocalStateTask(projPref, this, _client);
		// ex.execute(clst);
		//
		// for (final DataSource source : projPref.getDataSources()) {
		// final CalculateRelationshipTask crt = new
		// CalculateRelationshipTask(source, projPref, this, _client);
		// ex.execute(crt);
		// }
		// }
	...
	/**
	 * Either creates a new one (if one did not exist) or displays the existing
	...
	/**
	 * Updates the images and tool tips of all the projects and all the
	 * repositories within the current configuration.
	 */
	// @Override
	public void update() {
	...
	/**
	 * Updates the tray icon image to the harshest relationship in the current
	 * configuration.
	 */
	...
	public static void startLogging() {
		LSMRLogger.startLog4J(Constants.QUIET_CONSOLE, true, Constants.LOG_LEVEL, System.getProperty("user.home"),
				".conflictClientLog");
	}
}
