...
/**
 * A panel used for the configuration editor (PreferencesGUIEditorFrame) to
...
public class ProjectPanel extends JPanel {
	 ...
	 * 
	 * @param: copyPref
	 *             : the project configuration to display
	 * @param: copyPrefs:
	 *             the overall configuration associated with this project
	 * @param mainFrame:
	 *            the frame that will keep this panel
	 * @param tabbedPane:
	 *            the pane on the mainFrame that will keep this panel
	 * @param prefs
	 *            the original overall configuration
	 * 
	 */
	public ProjectPanel(final ProjectPreferences copyPref, final ClientPreferences copyPrefs, final JFrame mainFrame,
			final JTabbedPane tabbedPane, final Map<JComponent, Boolean> changedComponents,
			final ProjectPreferences pref, final Map<JTextField, Boolean> validText) {
		super();
		...
		shortName.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent arg0) {
				...
				((JLabel) ((JPanel) tabbedPane.getTabComponentAt(tabbedPane.getSelectedIndex())).getComponent(0))
						.setText(_name);

				if (pref != null) {
					changedComponents.put(shortName, !shortName.getText().equals(pref.getEnvironment().getShortName()));
				}
		...
		parentName.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent arg0) {
				...
				if (pref != null) {
					changedComponents.put(parentName, !parentName.getText().equals(pref.getEnvironment().getParent()));
				}
				// prefs.setChanged(true);
				panel.validate();
				mainFrame.pack();
		...
		address.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if (pref != null) {
					changedComponents.put(address, !address.getText().equals(pref.getEnvironment().getCloneString()));
				}
		...
		addressButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				// new MyPathChooser("Path to scratch directory", address,
		...
		test.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if (pref != null) {
					changedComponents.put(test, !test.getText().equals(pref.getEnvironment().getTestCommand()));
				}
				boolean testValid = ValidInputChecker.checkCommand(test.getText());
				validText.put(compile, testValid);
		...
		newRepoButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				addRepoPanel(newGuy, copyPref, copyPrefs, panel, mainFrame, sourcesPanel, changedComponents, null,
						validText);
		...
		for (DataSource copySource : copyPref.getDataSources()) {
			addRepoPanel(copySource, copyPref, copyPrefs, panel, mainFrame, sourcesPanel, changedComponents,
					pref.getDataSource(copySource.getShortName()), validText);
		}
	 ...
	 * @param panel:
	 *            the panel in which this panel sits
	 * @param mainFrame:
	 *            the frame in which this panel sits
	...
	private void addRepoPanel(final DataSource copySource, final ProjectPreferences copyPref,
			final ClientPreferences copyPrefs, final JPanel panel, final JFrame mainFrame, final JPanel sourcesPanel,
		...
		/*
		 * repoPanel.add(new JLabel("Repo Type")); final JComboBox type = new
		 * JComboBox(); type.addItem(DataSource.RepoKind.HG); //
		 * type.addItem(DataSource.RepoKind.GIT);
		 * type.setSelectedItem(source.getKind()); repoPanel.add(type);
		 * type.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { source.setKind((DataSource.RepoKind)
		 * type.getSelectedItem()); prefs.setChanged(true); panel.validate();
		 * mainFrame.pack(); } });
		 */
		...
		shortName.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if (source != null) {
					changedComponents.put(shortName, !shortName.getText().equals(source.getShortName()));
				}
		...
		cloneAddress.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent arg0) {
				...
				if (source != null) {
					changedComponents.put(cloneAddress, !cloneAddress.getText().equals(source.getCloneString()));
				}
		...
		deleteRepoButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int n = JOptionPane.showConfirmDialog(null, "Delete " + copySource.getShortName() + "'s repository?");
